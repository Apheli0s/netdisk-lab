...
private:
    ...
    void login(const TcpConnectionPtr &conn, string account, string cipher)
    {
        ...
        if (cipher.empty())
        {
            ...
            if (salt.empty())
                msg = "{\"command\":\"message\",\"data\":\"Account is not exist.\"}";
            else
                msg = "{\"command\":\"login\",\"salt\":\"" + salt + "\"}";
        }
        else
        {
            string s;
                ...
                else
                {
                    ...
                    if (res.empty())
                    {
                        ...
                        else
                        {
                            ...
                            sql = "INSERT INTO File_sys(Folder,FileName,FileSize,FileType,MD5,Account) VALUES('" + to_string(Folder) + "','" + FileName + "','" + to_string(members[4].v.u.n) + "','f','" + md5 + "','" + Account + "')";
                            db.exeSQL(sql);
